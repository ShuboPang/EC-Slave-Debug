...
int ec_firm_update(int slave,const char* filebuffer,int32 filesize){
    ...
    uint32 data = ec_readeeprom(slave, ECT_SII_BOOTRXMBX, EC_TIMEOUTEEP);
    ...
    if (ec_statecheck(slave, EC_STATE_BOOT,  EC_TIMEOUTSTATE * 10) == EC_STATE_BOOT)
    {
        ...
        printf("FoE write....");
        int j = ec_FOEwrite(slave, "slave", 0, filesize , &filebuffer, EC_TIMEOUTSTATE);
        printf("result %d.\n",j);
